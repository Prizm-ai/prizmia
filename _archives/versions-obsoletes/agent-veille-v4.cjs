// agent-veille-v4-complete.js - Version complète avec toutes les méthodes
...
const MASTER_PROMPT_VEILLE_V4 = `Tu es un expert en veille technologique spécialisé dans l'IA et la transformation digitale des PME/ETI françaises.
...
Format de sortie : Markdown structuré avec métadonnées complètes.`;
...
╚═══════════════════════════════════════════════════════╝
    ...
    try {
      ...
      // IMPORTANT : TOUJOURS créer le corpus pour chaque sujet retenu
      ...
      console.log(`\n✅ Veille terminée : ${this.sujetsRetenus.length} sujets avec corpus`);
  ...
  async lancerRecherche(options) {
    console.log('\n🔍 Lancement de la recherche Perplexity...\n');
  ...
  async parserSujetsAvecSources(contenu) {
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      // Extraire la synthèse et les angles
  ...
  async sauvegarderVeille(contenu, sujets) {
    ...
    // Ajouter l'en-tête
    let contenuFinal = `# 📊 VEILLE PRIZM AI v4 - ${date.toLocaleDateString('fr-FR', { 
      weekday: 'long', 
      year: 'numeric', 
      month: 'long', 
      day: 'numeric' 
    ...
    contenuFinal += `*Générée avec collecte multi-sources et corpus vérifié*\n\n`;
    contenuFinal += `---\n\n`;
    contenuFinal += `## 🎯 Synthèse exécutive\n\n`;
    contenuFinal += `${sujets.length} sujets analysés avec ${sujets.reduce((acc, s) => acc + s.sources.length, 0)} sources vérifiées.\n`;
...
Collecte et vérifie des sources multiples pour chaque sujet.
...
Fonctionnement :
  1. Recherche 5 sujets d'actualité IA
  2. Collecte 3-5 sources par sujet
  3. Vérifie la qualité des sources
  4. Crée un corpus structuré
  5. Prépare pour la rédaction factuelle
    `);
    process.exit(0);
  }
  ...
  try {
    ...
    // IMPORTANT : Créer le corpus pour chaque sujet
    if (resultat.sujets && resultat.sujets.length > 0) {
      console.log('\n📚 Création des corpus pour les sujets retenus...');
