// agent-veille-v5.cjs - Version hybride avec mode auto et mode dirigé
...
const CONFIG = {
  ...
  SOURCE_TYPES: {
    NEWS: { weight: 0.3, keywords: ['actualité', 'news', 'annonce'] },
    STUDY: { weight: 0.4, keywords: ['étude', 'rapport', 'analyse'] },
    CASE: { weight: 0.3, keywords: ['cas client', 'retour expérience', 'exemple'] }
  }
};

// Prompt pour le mode AUTO (comportement actuel)
const MASTER_PROMPT_AUTO = `Tu es un expert en veille technologique spécialisé dans l'IA et la transformation digitale des PME/ETI françaises.

MISSION : Identifier et analyser 5 sujets d'actualité IA pertinents pour des dirigeants de PME/ETI, en collectant 3-5 sources vérifiables par sujet.

CRITÈRES DE SÉLECTION :
1. Actualité récente (< 7 jours idéalement)
2. Impact business concret et mesurable
3. Applicable aux PME/ETI françaises
4. Sources fiables et vérifiables

DOMAINES PRIORITAIRES :
1. IA générative et productivité
2. Automatisation des processus
3. Cybersécurité et IA
4. Formation et compétences IA
5. Retours d'expérience PME
6. Financements et aides publiques
7. Réglementation et conformité IA`;

// Prompt pour le mode DIRIGÉ (nouveau)
const MASTER_PROMPT_DIRIGE = `Tu es un expert en veille technologique spécialisé dans l'IA et la transformation digitale des PME/ETI françaises.

MISSION : Réaliser une veille approfondie sur le sujet spécifique suivant :
"{SUJET}"
...
4. Fournir des données chiffrées et des exemples concrets

ANGLE ÉDITORIAL : {ANGLE}

MOTS-CLÉS POUR LA RECHERCHE : {KEYWORDS}`;
...
### Sources vérifiées
**Source 1** : [Nom de la source] — [Type : Article/Étude/Rapport]
- URL : [URL complète et exacte]
- Date : [JJ/MM/AAAA]
- Fiabilité : [X/10] ([justification courte])
- Extraits clés (MINIMUM 5 extraits détaillés) :
  * "[Citation exacte 1]"
  * "[Citation exacte 2]"
  * "[Citation exacte 3]"
  * "[Citation exacte 4]"
  * "[Citation exacte 5]"
- Données factuelles : [Chiffres, pourcentages, montants avec contexte]

[Répéter pour chaque source]

### Synthèse et angles
- **Convergences** : [Points communs entre sources]
- **Divergences** : [Différences notables]
- **Angle Prizm recommandé** : [Approche business pour PME/ETI]
- **Score global** : [X/10]

RÈGLES IMPÉRATIVES :
- MINIMUM 3 sources par sujet, idéalement 4-5
- URLs complètes et fonctionnelles obligatoires
- Citations exactes entre guillemets (5 minimum par source)
- Dates précises pour chaque source
- Fiabilité évaluée objectivement`;
...
class AgentVeilleV5 {
  ...
  /**
   * Point d'entrée principal avec support des deux modes
   * @param {Object} options - Options de configuration
   * @param {string} options.mode - 'AUTO' ou 'DIRIGE'
   * @param {Object} options.sujet - Sujet imposé pour le mode DIRIGE
   */
  async executer(options = {}) {
    ...
    console.log(`
╔════════════════════════════════════════════════════════╗
║            AGENT VEILLE v5 - MODE ${this.mode.padEnd(7)}          ║
║         Collecte et vérification des sources           ║
╚════════════════════════════════════════════════════════╝
    `);

    if (this.mode === 'DIRIGE' && this.sujetImpose) {
      console.log(`\n🎯 Sujet imposé : ${this.sujetImpose.titre}`);
      console.log(`   Catégorie : ${this.sujetImpose.category || 'actualites'}`);
    ...
    try {
      ...
      console.log(`\n✅ Veille terminée : ${this.sujetsRetenus.length} sujet(s) avec corpus`);
  ...
  async lancerRecherche(options) {
    console.log(`\n🔍 Lancement de la recherche Perplexity (mode ${this.mode})...\n`);
    
    let promptFinal;
    
    if (this.mode === 'DIRIGE' && this.sujetImpose) {
      // Mode dirigé : recherche sur le sujet imposé
      ...
      if (options.complet) {
        promptFinal += '\n\nRecherche APPROFONDIE sur TOUS les domaines listés.';
      } else {
      ...
      const response = await axios.post(
        PERPLEXITY_API_URL,
        {
          ...
          messages: [
            {
              role: 'system',
              content: 'Tu es un expert en veille IA pour PME/ETI. Fournis TOUJOURS des URLs complètes, AU MINIMUM 5 citations longues par source, et 3-4 données chiffrées détaillées.'
            },
            {
              role: 'user',
              content: promptFinal
  ...
  /**
   * Parser les résultats selon le mode
   */
  async parserResultats(contenu) {
    if (this.mode === 'DIRIGE') {
      // En mode dirigé, on a UN seul sujet avec plusieurs sources
  ...
  /**
   * Parser pour le mode DIRIGÉ (un seul sujet)
   */
  async parserSujetUnique(contenu) {
    console.log('📋 Parsing du sujet dirigé et ses sources...\n');
    ...
    if (sources.length >= CONFIG.MIN_SOURCES && scoreQualite >= CONFIG.QUALITY_THRESHOLD) {
      ...
      console.log(`   ✅ Sujet validé (score: ${scoreQualite.toFixed(2)})`);
    } else {
      console.log(`   ⚠️ Qualité insuffisante mais on garde quand même en mode dirigé`);
      this.sujetsRetenus = [sujet];
    }
  ...
  async parserSujetsMultiples(contenu) {
    ...
    console.log('📋 Parsing des sujets et sources...\n');
    ...
    console.log(`   Sujets trouvés : ${matches.length}`);
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      console.log(`\n   📌 Sujet ${current.numero} : ${current.titre}`);
      ...
      if (sources.length >= CONFIG.MIN_SOURCES && scoreQualite >= CONFIG.QUALITY_THRESHOLD) {
        ...
        console.log(`      ✅ Sujet retenu (score: ${scoreQualite.toFixed(2)})`);
      } else {
        console.log(`      ❌ Sujet rejeté (sources: ${sources.length}/${CONFIG.MIN_SOURCES}, score: ${scoreQualite.toFixed(2)})`);
      }
    }
  ...
  // === MÉTHODES COMMUNES (reprises de la v4) ===
  
  extraireSources(contenuSujet) {
    const sources = [];
    const regexSource = /\*\*Source (\d+)\*\* : (.+?) — (.+?)\n([\s\S]+?)(?=\*\*Source \d+|### Synthèse|$)/g;
    ...
    while ((match = regexSource.exec(contenuSujet)) !== null) {
      ...
      // Extraire fiabilité
      const fiabiliteMatch = contenuSource.match(/- Fiabilité : (\d+)\/10/);
      ...
      const donneesMatch = contenuSource.match(/- Données factuelles : (.+)/);
      if (donneesMatch) {
  ...
  extraireSynthese(contenuSujet) {
    ...
    const syntheseMatch = contenuSujet.match(/### Synthèse et angles([\s\S]+?)$/);
    if (syntheseMatch) {
      ...
      const angleMatch = contenuSynthese.match(/\*\*Angle Prizm recommandé\*\* : (.+)/);
  ...
  async sauvegarderVeille(contenu, sujets) {
    ...
    // Ajouter l'en-tête
    let contenuFinal = `# 📊 VEILLE PRIZM AI v5 - ${date.toLocaleDateString('fr-FR', { 
      weekday: 'long', 
      year: 'numeric', 
      month: 'long', 
      day: 'numeric' 
    ...
    contenuFinal += `*Mode : ${this.mode} - Collecte multi-sources et corpus vérifié*\n\n`;
    
    if (this.mode === 'DIRIGE' && this.sujetImpose) {
      contenuFinal += `## 🎯 Sujet imposé\n`;
      contenuFinal += `**Titre** : ${this.sujetImpose.titre}\n`;
      contenuFinal += `**Catégorie** : ${this.sujetImpose.category || 'actualites'}\n`;
      contenuFinal += `**Angle** : ${this.sujetImpose.angle || 'Non spécifié'}\n\n`;
    }
    
    contenuFinal += `---\n\n`;
    contenuFinal += `## 🎯 Synthèse exécutive\n\n`;
    contenuFinal += `${sujets.length} sujet(s) analysé(s) avec ${sujets.reduce((acc, s) => acc + s.sources.length, 0)} sources vérifiées.\n`;
    contenuFinal += `Score de qualité moyen : ${(sujets.reduce((acc, s) => acc + s.scoreQualite, 0) / sujets.length).toFixed(2)}/1\n\n`;
    contenuFinal += `---\n\n`;
    
    contenuFinal += contenu;
    
    await fs.writeFile(filepath, contenuFinal, 'utf-8');
    console.log(`\n💾 Veille sauvegardée : ${filepath}`);
  ...
  async creerCorpusVerifie(sujet) {
    console.log(`\n📚 Création du corpus pour : ${sujet.titre}`);
    
    const date = new Date();
    const dateStr = date.toISOString().split('T')[0];
    
    // Créer un slug propre pour le dossier
    ...
    for (let i = 0; i < sujet.sources.length; i++) {
      ...
      contenuSource += `**Fiabilité** : ${source.fiabilite}/10\n\n`;
    ...
    console.log(`   ✅ Corpus créé : ${corpusDir}`);
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
📚 Agent Veille v5 - Mode Hybride
==================================

Collecte et vérifie des sources avec deux modes de fonctionnement.

Usage :
  node agent-veille-v5.cjs [options]                    # Mode AUTO
  node agent-veille-v5.cjs --dirige "titre" [options]   # Mode DIRIGÉ

Options :
  --auto       Mode automatique (défaut) : Perplexity choisit les sujets
  --dirige     Mode dirigé : impose un sujet spécifique
  --titre      Titre du sujet (obligatoire avec --dirige)
  --angle      Angle éditorial (optionnel)
  --keywords   Mots-clés pour la recherche (optionnel)
  --category   Catégorie : actualites|guides|analyses (défaut: actualites)
...
Exemples :
  ...
  # Mode DIRIGÉ (nouveau)
  node agent-veille-v5.cjs --dirige --titre "IA générative pour PME en 2025"
  node agent-veille-v5.cjs --dirige \\
    --titre "Microsoft Copilot vs ChatGPT Enterprise" \\
    --angle "Comparatif pour PME françaises" \\
    --keywords "Copilot, ChatGPT, comparatif, prix, fonctionnalités" \\
    --category "analyses"

Fonctionnement :
  Mode AUTO :
    1. Recherche 5 sujets d'actualité IA via Perplexity
    2. Collecte 3-5 sources par sujet
    3. Vérifie la qualité et filtre
    4. Crée les corpus pour les sujets retenus
    
  Mode DIRIGÉ :
    1. Recherche approfondie sur LE sujet imposé
    2. Collecte 4-5 sources spécifiques
    3. Crée un corpus détaillé
    4. Prépare pour la rédaction
    `);
    process.exit(0);
  }
  ...
  try {
    ...
    if (args.includes('--dirige')) {
      ...
      // Extraire les paramètres du sujet
      const titreIndex = args.findIndex(a => a === '--titre');
      if (titreIndex === -1 || !args[titreIndex + 1]) {
        console.error('❌ Le mode DIRIGÉ nécessite un titre (--titre "...")');
        process.exit(1);
      }
    ...
    console.time('⏱️ Durée totale');
    const resultat = await agent.executer(options);
    console.timeEnd('⏱️ Durée totale');
    
    console.log('\n✨ Veille terminée avec succès !');
    if (resultat.mode === 'DIRIGE') {
      console.log(`\n📄 Article à générer : ${resultat.sujets[0].titre}`);
      console.log(`📚 Corpus prêt : ${resultat.sujets[0].corpusPath}`);
    }
