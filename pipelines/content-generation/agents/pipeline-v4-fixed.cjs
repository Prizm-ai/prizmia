// pipeline-v4-fixed.js - Pipeline corrigé avec génération automatique de veille
...
// Configuration
// Configuration déplacée vers prizm-config.js
const CONFIG = {
  AGENTS: {
...
class PipelinePrizmV4 {
  ...
  async initialiser() {
    console.log(`
╔═══════════════════════════════════════════════════════╗
║              PIPELINE PRIZM AI v4                     ║
║          Double sortie : Factuel + Conversationnel    ║
║                                                       ║
...
╚═══════════════════════════════════════════════════════╝
  ...
  async creerStructure() {
    ...
    // Créer aussi les sous-dossiers nécessaires
    const date = new Date();
    const dateStr = date.toISOString().split('T')[0];
    
    // Créer le dossier de veille du jour
    const veilleJour = path.join('../output', CONFIG.STRUCTURE.veille, dateStr.substring(0, 7), dateStr);
  ...
  // === ÉTAPE 1 : VEILLE ET SÉLECTION (CORRIGÉE) ===
  async etapeVeille(forceNouvelle = false) {
    console.log(`
┌─────────────────────────────────────────┐
│     ÉTAPE 1 : VEILLE & SÉLECTION       │
└─────────────────────────────────────────┘
    `);
    ...
    // Si pas de nouvelle veille forcée, chercher la dernière
    if (!forceNouvelle) {
      console.log('📋 Recherche de la dernière veille...\n');
      ...
      try {
        ...
        if (fichierVeille) {
          console.log(`📄 Veille trouvée : ${path.basename(fichierVeille)}`);
          ...
          if (ageHeures > 24) {
            console.log(`⚠️  Veille datée de plus de 24h (${Math.round(ageHeures)}h)`);
            console.log('🔄 Génération d\'une nouvelle veille recommandée\n');
          }
        } else {
          console.log('❌ Aucune veille existante trouvée');
          doitGenererVeille = true;
        }
      } catch (error) {
        console.log('⚠️  Erreur recherche veille :', error.message);
        doitGenererVeille = true;
      }
    }
    
    // Générer une nouvelle veille si nécessaire
    if (doitGenererVeille) {
      console.log('🔍 Génération d\'une nouvelle veille...\n');
      ...
      try {
        ...
        if (stderr) {
          console.log('⚠️  Avertissements veille :', stderr);
        }
        
        // Attendre un peu que le fichier soit bien écrit
        await new Promise(resolve => setTimeout(resolve, 1000));
        
        // Retrouver le fichier généré
        fichierVeille = await this.trouverDerniereVeille();
        
        if (!fichierVeille) {
          throw new Error('Fichier de veille généré mais introuvable');
        }
        ...
        console.log(`\n✅ Nouvelle veille générée en ${Math.round((Date.now() - debut) / 1000)}s`);
        
      } catch (error) {
        console.error('❌ Erreur génération veille :', error.message);
        this.stats.etapes.push({
          nom: 'veille',
          duree: Date.now() - debut,
          succes: false,
    ...
    console.log(`\n📄 Utilisation de : ${path.basename(fichierVeille)}`);
    ...
    if (!resultatParse || !resultatParse.sujets || resultatParse.sujets.length === 0) {
      throw new Error('Aucun sujet trouvé dans la veille');
    }
    
    const sujets = resultatParse.sujets;
    console.log(`\n📊 ${sujets.length} sujets trouvés`);
    ...
    sujetsScores.slice(0, 3).forEach((sujet, i) => {
      ...
      if (sujet.priorite) {
        console.log(`   Priorité : ${sujet.priorite}`);
      }
    });
    
    // Sélection du meilleur sujet
  ...
  async trouverDerniereVeille() {
    const dossierVeille = path.join('../output', CONFIG.STRUCTURE.veille);
    ...
    console.log(`   Dernière veille : ${tousLesFichiers[0].date}`);
  ...
  async selectionnerSujet(sujets, fichierVeille) {
    ...
    try {
      ...
      if (dossierSujet) {
        ...
        console.log(`📚 Corpus trouvé : ${dossierSujet}`);
      } else {
        console.log('⚠️  Corpus non trouvé, génération avec le titre seul');
      }
    } catch (error) {
      console.log('⚠️  Pas de corpus disponible pour ce sujet');
    }
  ...
  // [Copier toutes les autres méthodes depuis le pipeline-v4.js original]
  
  async etapeRedactionFactuelle(sujet) {
    console.log(`
┌─────────────────────────────────────────┐
│     ÉTAPE 2 : RÉDACTION FACTUELLE      │
└─────────────────────────────────────────┘
    `);
    ...
    try {
      ...
      // Trouver l'article généré
      const dossierFactuels = path.join('../output', CONFIG.STRUCTURE.factuels);
      ...
      if (!dernierArticle) {
        throw new Error('Article factuel non généré');
      }
      ...
      console.log(`\n✅ Article factuel généré : ${dernierArticle}`);
      console.log(`📏 Longueur : ${validation.mots} mots ${validation.statut}`);
      ...
      console.error('❌ Erreur rédaction :', error.message);
      this.stats.etapes.push({
        nom: 'redaction-factuelle',
        duree: Date.now() - debut,
        succes: false,
  ...
  async etapeStyleConversationnel(articleFactuel, activer = true) {
    if (!activer) {
      console.log('\n⏭️  Style conversationnel désactivé');
      return null;
    }
    
    console.log(`
┌─────────────────────────────────────────┐
│   ÉTAPE 3 : STYLE CONVERSATIONNEL      │
└─────────────────────────────────────────┘
    `);
    ...
    try {
      ...
      const dossierFinaux = path.join('../output', CONFIG.STRUCTURE.conversationnels);
      ...
      if (!articleConversationnel) {
        throw new Error('Article conversationnel non généré');
      }
      ...
      console.log(`\n✅ Article conversationnel généré : ${articleConversationnel}`);
  ...
  async validerArticle(cheminArticle) {
    ...
    if (validation.longueurOk && validation.sources && validation.structure) {
      ...
      validation.statut = '⚠️ (trop court)';
    } else if (mots > CONFIG.VALIDATION.longueurMax) {
      validation.statut = '⚠️ (trop long)';
    } else {
      validation.statut = '❌ (structure)';
    }
...
└─────────────────────────────────────────┘
  ...
  async genererRapport() {
    ...
    console.log(`\n📊 Rapport généré : ${path.basename(cheminRapport)}`);
    
    // Version Markdown lisible
    let md = `# Rapport Pipeline v4 - ${this.sessionId}\n\n`;
    md += `**Date** : ${new Date().toLocaleString('fr-FR')}\n`;
    md += `**Durée totale** : ${Math.round((Date.now() - this.stats.debut) / 1000)}s\n\n`;
    
    md += `## Résultats\n\n`;
    md += `### Veille\n`;
    md += `- Fichier : ${this.stats.resultats.veille?.fichier || 'N/A'}\n`;
    md += `- Sujets analysés : ${this.stats.resultats.veille?.nbSujets || 0}\n`;
    ...
    md += `\n## Performance\n\n`;
    md += `| Étape | Durée | Statut |\n`;
    md += `|-------|-------|--------|\n`;
    this.stats.etapes.forEach(etape => {
      md += `| ${etape.nom} | ${Math.round(etape.duree / 1000)}s | ${etape.succes ? '✅' : '❌'} |\n`;
    });
  ...
  async archiverSession() {
    ...
    console.log(`📦 Session archivée : ${path.basename(this.dossierSession)}`);
  }
...
────────────────────────
⏱️  Durée totale : ${dureeTotal}s
📄 Articles générés : ${articlesFinaux.length}
`);

    articlesFinaux.forEach(article => {
      console.log(`
${article.type === 'factuel' ? '📚' : '🗣️'} Article ${article.type} :
   ${article.fichier}
`);
    });
...
2. Choisir la version adaptée à votre objectif :
   - Factuel → Pour informer (blog référence, documentation)
   - Conversationnel → Pour engager (newsletter, réseaux sociaux)
3. Copier dans le blog : C:\\Users\\Samuel\\Documents\\prizmia\\src\\content\\blog\\
4. Publier via git push

✨ Pipeline v4 - Simple, efficace, flexible.
`);
  }
  ...
  async executer(options = {}) {
    try {
      ...
      // Étape 3 : Style conversationnel (optionnel)
...
async function main() {
  ...
  if (args.includes('--help') || args.includes('-h')) {
    console.log(`
🚀 Pipeline Prizm AI v4 (CORRIGÉ)
=================================

Pipeline épuré avec double sortie : factuel et conversationnel.
Génère automatiquement une veille si aucune n'existe.

Usage :
  node pipeline-v4-fixed.js [options]

Options :
  --nouvelle-veille    Forcer une nouvelle veille (même si une existe)
  --factuel-only       Générer uniquement l'article factuel
  --help, -h           Afficher cette aide

Exemples :
  node pipeline-v4-fixed.js                    # Utilise la dernière veille OU en génère une
...
Comportement par défaut :
  1. Cherche la dernière veille disponible
  2. Si aucune veille → génère automatiquement
  3. Si veille > 24h → propose une nouvelle
  4. Génère article factuel + conversationnel
...
Temps moyen : ~2-3 minutes pour pipeline complet
  ...
  try {
    console.time('⏱️ Pipeline complet');
    await pipeline.executer(options);
    console.timeEnd('⏱️ Pipeline complet');
